...
import {AlertCircle, CheckCircle, FolderOpen, Save, Trash2, X} from 'lucide-react';
...
export const StorageModals = ({mngr, theme}) => {
    ...
    return (
        <>
            ...
            {notification && (
                <div
                    className={`fixed bottom-8 right-8 px-6 py-3 rounded shadow-2xl flex items-center space-x-3 z-50 border bg-black/95 ${
            ...
            {storageModalOpen && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className={`bg-black border rounded p-6 max-w-lg w-full relative ${borderText}`}>
                        <button onClick={() => setStorageModalOpen(false)}
                        ...
                        <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
                            ...
                            {savedChars.map(c => (
                                <div key={c.id}
                                     className="flex items-center justify-between p-3 border border-white/20 bg-black/40 rounded">
                                    <div>
                                        <div className="font-bold text-sm">{c.name}</div>
                                        <div
                                            className="text-[10px] uppercase tracking-wider opacity-60">{c.subInfo} • {c.date}</div>
                                    </div>
                                    <div className="flex space-x-2">
                                        <button onClick={() => loadFromLocal(c.data)}
                                                className="p-2 hover:bg-white/20 rounded">
                                            <FolderOpen size={16}/>
                                        </button>
                                        <button onClick={() => deleteFromLocal(c.id)}
                                                className="p-2 text-red-500 hover:bg-red-900/40 rounded">
                                            <Trash2 size={16}/>
                                        </button>
                                    </div>
                                </div>
